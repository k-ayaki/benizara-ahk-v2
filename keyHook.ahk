;-----------------------------------------------------------------------
;	名称：keyHook.ahk (AHK v2版)
;	機能：キーフック
;	作者：Ken'ichiro Ayaki
;-----------------------------------------------------------------------
#Requires AutoHotkey v2.0
;----------------------------------------------------------------------
; キーを押下されたときに呼び出される関数にフックする
;----------------------------------------------------------------------
SetHotkeyInit()
{
	global g_Hotkey, g_HotkeyFunction, g_HotkeyNumpad
	...
	SetHotkeyInitByPos("G06")	;PageUp
	SetHotkeyInitByPos("G07")	;Delete
	SetHotkeyInitByPos("G08")	;End
	SetHotkeyInitByPos("G09")	;PageDn
...
{
	global keyHook, ScanCodeHash
	
	_sCode := ScanCodeHash.Has(_pos) ? ScanCodeHash[_pos] : ""
	if (_sCode != "") {
		keyHook[_pos] := "Off"
		; 第2引数に文字列ではなく、直接コールバック関数を指定します
		Hotkey("*" _sCode, GlobalKeyDownCallback)
		Hotkey("*" _sCode, "Off")
		Hotkey("*" _sCode " up", GlobalKeyUpCallback)  ; ← " up" の半角スペースも忘れずに！
...
{
	global g_HotkeyFunction
    
    ; 大文字小文字の違いを吸収するため "=" を使用
	if (_flg = "On" || _flg = "Off")
	{
...
SetHotkeyNumpad(_flg)
{
	...
	if (_flg = "On" || _flg = "Off") 
	{
		if (g_HotkeyNumpad != _flg) ; on-offが変化したときだけ実行
		{
			...
			SetHotkeyFunctionByName("NumpadEnter", _flg)
...
{
	global fkeyPosHash, fkeyCodeHash, keyAttribute3, keyHook
	
	_pos := fkeyPosHash.Has(_kName) ? fkeyPosHash[_kName] : ""
	_sCode := fkeyCodeHash.Has(_kName) ? fkeyCodeHash[_kName] : ""
    ...
    _attrKey := State.Romaji . KoyubiOrSans(State.Koyubi, State.Sans) . _pos
	if (keyAttribute3.Has(_attrKey) && keyAttribute3[_attrKey] != "") {
		Hotkey("*" _sCode, _flg)
		keyHook[_pos] := _flg
...
{
	global ScanCodeHash, keyAttribute3, kup_save, keyHook
	...
	if (keyAttribute3.Has(_attrKey) && keyAttribute3[_attrKey] != "") {
		Hotkey("*" _sCode, _flg)
		if (!kup_save.Has(_pos) || kup_save[_pos] == "")
		{
			keyHook[_pos] := _flg
			Hotkey("*" _sCode " up", _flg)
		}
